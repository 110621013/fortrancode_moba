        ...
        dimension pp(9999),zz(9999),tt(9999),td(9999),wd(9999),ws(9999)
        character station*10,time*8
        character indir*20,infile*20
        real p,ht,t,dp,dir,spd,rh 
         !pressure, height, temperature, dewpoint, wind direction, wind speed, relative humidity 
        integer a,e,f

c-----------------------------------------------------------------------
c* To open souding data.
      indir='../'
      infile='4669902014081900.txt'
      open (10,file=trim(indir)//trim(infile),form='formatted',err=100)
      wsps=20
c-----------------------------------------------------------------------
c  read souding data.

      station='466990'
      write(time,'(a)')"14081900"

      nn=0
  10  continue
      read(10,*,err=10,end=20)a,f,e,p,ht,t,dp,dir,spd,rh
      if((p.gt.2000.).or.(p.lt.0))goto 10
      if((ht.ge.90000.).or.(ht.lt.0))ht=-9999.
      if((t.ge.900.))t=-9999.
      if((dp.ge.900.))dp=-9999.
      if((spd.ge.900.).or.(spd.lt.0))spd=-9999.
      if((dir.ge.900.).or.(dir.lt.0))dir=-9999.
      if((rh.gt.100.))then
         rh=100.
      else
         rh=rh
      endif
!      dp=ptrh2td(p,t,rh)    !if you don't have dewpoint data, use this subroutine to calculate
      nn=nn+1
      pp(nn)=p
      zz(nn)=ht
      tt(nn)=t
      ...
      wd(nn)=dir
      ws(nn)=spd
!      print*,nn,zz(nn),pp(nn),tt(nn),td(nn),wd(nn),ws(nn)
      go to 10
  20  continue

c     call sort(pp,zz,tt,td,wd,ws,nn)
...
c  plot skewT diagram.
 ...
 300    continue
     ...
     +             wd,ws,wsps,ier)
       ...
       character station*10,time*8
       dimension xx(9000),yy(9000),wp(9000),ip(10)
       dimension p_a(9000),t_a(9000),t_e(9000)
...
c* To close G.K.S.
      ...
      function pt2q(p,t)
c
c  p(mb), t(deg. c) find mixing ratio (kg/kg)
c  bolton's formula 1980 wmr p.1048
c
      es=6.112*exp(17.67*t/(t+243.5))
      pt2q=0.62197*es/(p-es)
      return
      end
      function pq2t(p,q)
c
c  p(mb), q(kg/kg), find saturated temperature (c)
c  bolton's formula
c
      data tmis/-999.0/
      e=p*q/(q+0.62197)
      if(e.lt.0.0001)then
         pq2t=tmis
         print*,'hard to find temp in pq2t',p,q,pq2t,char(7)
      else
         b=alog(e/6.112)/17.67
         pq2t=b*243.5/(1.-b)
      endif
      return
      end
      function ptrh2td(p,t,rh)
c
c  p(mb), t(c), rh(%) find dew point td(c)
c
      qs=pt2q(p,t)
      q=qs*rh/100.
      ptrh2td=pq2t(p,q)
...
c I/O parameters   :                                                   *
c  input:  name       type        description                          *
c          station    C*10         the station number.                  *
...
c***********************************************************************
       ...
       character station*10,time*8,ca*20
...
c* To write station number.
       ...
       ca(1:10)='STATION = '
       ca(11:20)=station(1:10)
       call plchhq(25.,2.5,ca,0.012,0.,0.)
...
c***********************************************************************
       ...
       do 10 k=1,nn
          ...
          if(abs(rlog_p-pplt).ge.plsps)then
             ...
             call plchhq(x,y,':KRL:E',0.0025,0.,0.)
...
!#include </usr/local/ncarg/include/ncarg/ncargC.h>
