...
*** draw color bar ***
    if( edge = 'box' )
      ...
      if( line = 'on' ) ; drawrec( linecolor, x1, y1, x2, y2 , lt ) ; endif
    endif
    ...
    if( edge = 'triangle' )
      ...
      if( direction = 'horizontal' )
        if( i = 1 )
          ...
          'draw polyf 'poly
          if( line = 'on' ) ; drawpoly( linecolor' 'poly' 'lt ) ; endif
        endif
        if( i = cnum )
          ymed = 0.5 * ( y1 + y2 )
...
*exit
          ...
          'draw polyf 'circle
          if( line = 'on' ) ; drawpoly( linecolor' 'x2' 'y2' 'circle' 'x2' 'y1' 'lt ) ; endif
        endif
        if( i = cnum )
          xc = x2 - ( y2 - y1 )
          yc = y1 + 0.5 * ( y2 - y1 )
          ...
          'draw polyf 'circle
          if( line = 'on' ) ; drawpoly( linecolor' 'x1' 'y1' 'circle' 'x1' 'y2' 'lt ) ; endif
        endif
      endif
      ...
      if( direction = 'vertical' )
        if( i = 1 )
          ...
          'draw polyf 'circle
          if( line = 'on' ) ; drawpoly( linecolor' 'x1' 'y2' 'circle' 'x2' 'y2' 'lt ) ; endif
        endif
        if( i = cnum )
          xc = x1 + 0.5 * ( x2 - x1 )
          yc = y2 - 0.5 * ( x2 - x1 )
          ...
          'draw polyf 'circle
          if( line = 'on' ) ; drawpoly( linecolor' 'x2' 'y1' 'circle' 'x1' 'y1' 'lt ) ; endif
        endif
      endif
    endif
*   end of circle
...
function drawrec( linecolor, xmin, ymin, xmax, ymax , lt )
  drawpoly( linecolor' 'xmin' 'ymin' 'xmin' 'ymax' 'xmax' 'ymax' 'xmax' 'ymin' 'lt )
return
...
function help()
  ...
  say '     fthickness : font thickness (default=fheight*30)'
  ...
  say '     lt         : thickness of line (default=1)'
